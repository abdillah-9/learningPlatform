    ...
    async function FetchAllUpcomingCourses(page = 1) {
      ...
      try{
        const res = await fetch(
          "https://www.tanzcoffee.co.tz/mwangaza_hub/fetch_upcoming_courses.php",
...
function CoursesDetails({formState, setFormState, setEditModeState, courseData, setCourseData, FetchAllUpcomingCourses, fetchedCourses, currentPage,
  ...
  async function HandleDeleteCourse(upcoming_course_id){
    ...
    const res = await fetch('https://www.tanzcoffee.co.tz/mwangaza_hub/delete_upcoming_course.php',{
      body:formData,
      method:"post"
    });
    if(res.ok){
        ...
        <div style={{width:'100%', display:'flex', gap:"15px", flexDirection:'column' }}>
          {fetchedCourses && fetchedCourses?.map((course, index)=>(
            ...
            maxWidth:'900px',
              ...
              <img src={'https://www.tanzcoffee.co.tz/mwangaza_hub/'+(course.course_photo ? course.course_photo : "uploads/noPhoto.jpeg")} alt="pic" style={{width:'40%', maxWidth:'250px', aspectRatio:16/9, objectFit:'cover'}}/>
...
function Form({formState, setFormState, editModeState, courseData, FetchAllUpcomingCourses}){
  ...
  async function handleSubmit(e){
     ...
     try{
      ...
      const res = await fetch('https://www.tanzcoffee.co.tz/mwangaza_hub/create_edit_upcoming_course.php',{
        method:'POST',
        body:formData,
      });
        ...
        <form onSubmit={handleSubmit} style={{
            ...
            <div style={{display:'flex', gap:'15px', alignItems:'center', borderBottom:'1px solid rgba(29, 61, 69, 0.64)', justifyContent:'space-between', paddingBottom:'15px', flexWrap:'wrap'}}>
                ...
                }}/>
                {
                  ...
                  <img src={courseData ? 'https://www.tanzcoffee.co.tz/mwangaza_hub/'+courseData.course_photo : ""} alt='pic' style={{objectFit:'cover', width:'50%', aspectRatio:16/9, boxShadow:'1px 2px 3px black'}}/> :""
                }
            </div>
            <div style={{display:'flex', gap:'15px', alignItems:'center', borderBottom:'1px solid rgba(29, 61, 69, 0.64)', justifyContent:'space-between', paddingBottom:'15px', flexWrap:'wrap'}}>
