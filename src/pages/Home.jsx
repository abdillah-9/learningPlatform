...
/>
      ...
      {/* Heading */}
      <div className={`slideTop PicTop`} style={{ ...topSlideshowTexts, color: 'white',fontWeight:500, border:'1px solid red', display:'flex', alignItems:'center', flexDirection:"column" }}>
        <div className="PicFont" style={{fontWeight:700, marginBottom:'20px', width:'70%', maxWidth:'400px'}}>{headings[index]}</div>
        <div className="PicFontMini" style={{width:'70%',maxWidth:'400px'}}>{texts[index]}</div>
      </div>

      {/* NEXT/PREV BUTTONS */}
      <div className="PicTop" style={{ position: "absolute",left: '2%', transform: "translateY(-50%)", zIndex: 10, borderRadius: '50%', padding: '10px 13px', backgroundColor: '#253957b2', cursor: 'pointer' }} onClick={prev}>
        <CgChevronDoubleLeft style={{ fontSize: '25px', color: 'white' }} />
      </div>

      <div className="PicTop" style={{ position: "absolute",right: '2%', transform: "translateY(-50%)", zIndex: 10, borderRadius: '50%', padding: '10px 13px', backgroundColor: '#253957b2', cursor: 'pointer' }} onClick={next}>
...
function MiniSlideShow({ items, decodeHTML }) {
  ...
  return (
    <div style={{display:'flex', justifyContent:'center', gap:'25px', flexWrap:'wrap',padding:'0px 15px'}}>
      {items.map((item, i) => (
        <div key={i} className="slideShowHeight" style={{ minWidth:'220px',width: "30%", position: "relative", overflow: "hidden",aspectRatio:1/0.85, borderRadius:'5px'}}>
          ...
          <div style={{ ...miniSlideshowTexts, color: 'white', backgroundColor:'#253957', width:'100%', display:'flex', flexDirection:'column', gap:'10px', padding:'10px 10px 20px 10px'}} className="heightHover" onClick={()=>{navigateTo(`enroll_course/${item.id}`)}}>
